...
thicc-svg is a monorepo with two packages:

- **`packages/engine`** — `<SVG3D>` React component published to npm as `thicc-svg`
- **`packages/web`** — Next.js visual editor at thicc-svg.design
...
- Web imports engine as a workspace dependency (`"thicc-svg": "file:../engine"`)
