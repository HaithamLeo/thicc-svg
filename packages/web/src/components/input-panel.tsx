...
import { Pencil, Type, FileUp, Upload, Code, FileCheck, X } from "lucide-react";
...
export function InputPanel({
  ...
  return (
    <div ref={panelRef} className="flex items-start gap-2 pointer-events-none">
      {/* Vertical toolbar */}
      <div className="relative flex flex-col gap-1 rounded-xl bg-card/70 backdrop-blur-2xl border border-primary/[0.1] shadow-[0_0_30px_oklch(0.78_0.2_185/0.06),0_8px_40px_oklch(0_0_0/0.5)] p-1.5 pointer-events-auto overflow-hidden">
        {/* Top neon accent line */}
        <div className="absolute top-0 left-2 right-2 h-[1px] bg-gradient-to-r from-transparent via-primary/40 to-transparent" />
        {tabs.map((tab) => {
          const Icon = tab.icon;
              ...
              <TooltipTrigger asChild>
                <Button
                  ...
                  className={`h-9 w-9 transition-all duration-200 ${
                    isOpen
                      ? "bg-primary/15 text-primary ring-1 ring-primary/40 shadow-[0_0_10px_oklch(0.75_0.18_195/0.15)]"
                      : isSelected
      ...
      <motion.div
        animate={
          expanded
            ...
            : { opacity: 0, x: -8, pointerEvents: "none" as const }
        }
        transition={{ duration: 0.18, ease: [0.4, 0, 0.2, 1] }}
        className="relative w-80 rounded-xl bg-card/70 backdrop-blur-2xl border border-primary/[0.08] shadow-[0_0_30px_oklch(0.78_0.2_185/0.05),0_8px_40px_oklch(0_0_0/0.5)] p-3 overflow-hidden"
      >
        {/* Top neon accent line */}
        <div className="absolute top-0 left-3 right-3 h-[1px] bg-gradient-to-r from-transparent via-primary/30 to-transparent" />
          ...
          <div className="space-y-2">
            ...
            {!customSvg && (
              <Button
                ...
                onClick={() =>
                  onCustomSvgChange(
                    '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" fill="black"/></svg>',
                  )
                }
              >
                Load example (star)
