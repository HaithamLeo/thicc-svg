...
interface DownloadDialogProps {
  ...
  onDownload: (format: Export3DFormat, meshOnly: boolean) => void;
}
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        ...
        <div className="flex items-center justify-between rounded-lg border border-white/[0.08] px-4 py-3">
          <div>
            <Label htmlFor="mesh-only" className="text-sm font-medium cursor-pointer">Mesh only</Label>
            <p className="text-[11px] text-muted-foreground mt-0.5">
              Geometry only — no materials or colors. Style it yourself in Three.js or Blender.
            </p>
          </div>
          <Switch
            id="mesh-only"
            checked={meshOnly}
            onCheckedChange={setMeshOnly}
          />
        </div>
