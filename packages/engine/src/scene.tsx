...
function parseShapesFromSVG(svgString: string, strokeScale = 1): THREE.Shape[] {
  ...
  svgData.paths.forEach((path) => {
    ...
    if (hasStroke) {
      const strokeWidth = parseFloat(style?.strokeWidth ?? "2") * strokeScale;
      const divisions = 12;
...
export function useExtrudedGeometry(
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const allShapes = parseShapesFromSVG(svgString, strokeScale);
  ...
  }, [svgString, depth, smoothness, strokeScale]);
...
export function ExtrudedSVG({
  ...
  const { geometries, center, baseScale, loading, progress } = useExtrudedGeometry(svgString, depth, smoothness, strokeScale);
