...
async function loadFontByName(name: string, url: string): Promise<Font> {
...
export function useFont(fontName: string): Font | null {
  const [loadedFont, setLoadedFont] = useState<Font | null>(() => {
...
export function textToSvg(text: string, font: Font): string {
  const size = 200;
